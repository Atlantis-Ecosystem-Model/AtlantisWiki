...
## Goals of this manual and where to go from here
...
1. The [**Atlantis Wiki**](https://atlantis-ecosystem-model.github.io/atlantis-wiki/) is the first place to go for further information. Also remember to look at the page on [*Frequently Asked Questions*](https://atlantis-ecosystem-model.github.io/atlantis-wiki/atlantis-faq.html) for troubleshooting issues.
...
#### Publications Explaining Processes Used in Atlantis (Initially called IGBEM and BM2)
...
**Routine** names are shown in *italics().* In computer programming, routines and subroutines are nearly synonymous terms describing a sequence of code that is intended to be called repeatedly during the execution of a program. A routine performs a specific task and is relatively independent of the remaining code (although it might use variables defined or calculated in other routines).
...
Atlantis parameters can be grouped into **flags** and **parameters** (in narrow sense). **Flags** activate various optional routines; they are used in the if statements in the code (if flagA=1 do routine aaa). For further information about what flags do, check this link to [flags in the biology parameter file](https://atlantis-ecosystem-model.github.io/atlantis-wiki/finding-all-the-options-for-the-flags.html) on the Atlantis wiki. There are no clear rules on naming of parameters and flags, so the name alone does not indicate whether it is a parameter or flag.

Atlantis requires a lot of input files. Some of them are used for external forcing of the model (@fig-atlantis-structure) and will be referred to as **inputs**. They are all listed in the *force.prm* file (note names of files used to run the model will be given simply in italics, to avoid confusing them with code files). Other input files define parameters or groups used in the submodels or simulation conditions and are referred to as **parameter files**. They are all listed in the BAT file used to initialise the runs.

::: {.callout-note}
Several parameter and group names in Atlantis contain typos that are baked into the source code. Users must spell these names exactly as shown in this manual — matching the code — even when the spelling appears wrong. Known cases include [temp_ampltiude]{style="color: orange;"}, [isLightEffected]{style="color: orange;"}, [isNoiseEffected]{style="color: orange;"}, and `MICROPHTYBENTHOS`.
:::
...
**Age groups** define age classes used in the model to represent fully age-structure groups (and can be set to anything from a few months to any number of calendar years). Many parameters for these groups are age-group-specific. All individuals in an age group are considered identical, unless the option of multiple genotypes is used 
, in which case all individuals within one genotype of one age group are identical.
...
## Existing applications and questions studied

Most Atlantis development and applications have centred on evaluating alternative management strategies for multispecies fisheries, or exploratory analyses of multiple ecosystem pressures. To date, the Atlantis framework has been applied to over 30 ecosystems around the world (@fig-atlantis-applications) primarily to study questions of:
...
![Some existing applications of Atlantis modelling framework and specific model domains (see @tbl-atlantis-applications for details on numbered models).](media/image2.png){#fig-atlantis-applications width=8in height=6in}
...
: Examples of Atlantis model applications and their main purposes: P = primary use of the model, S = secondary use, E = exploratory use beginning (modified after Fulton et al. 2011). Some of the model descriptions are included in the [useful reading](https://confluence.csiro.au/display/Atlantis/Useful+Reading) page on the wiki. An updated list of models under development is also available on Atlantis [webpage](http://atlantis.cmar.csiro.au/). {#tbl-atlantis-applications .small tbl-colwidths=[16,7,8,7,7,7,5,7,7,7,7,15]}
...
| **Central California** (ECCAL model) | • Fleets are differentially affected by changes in management rules or environmental conditions (some suffer significant economic declines while others see substantial increases)<br>• Shifting gears to minimise bycatch and habitat destruction does not restore age structures, but spatial closures can |
...
: Some example findings from Atlantis applications (modified after Fulton et al. 2011). {#tbl-atlantis-findings}

## How does Atlantis compare to other marine ecosystem models? {#sec-comp-es-models}
