# Processes in Bacterial and Inanimate Pools {#sec-bacterial-inanimate}
...
::: {.callout-note}
**Reasons for excluding bacteria in PPBIM (from Murray and Parslow 1997)**
...
$G_{BP} = mum \cdot B \cdot \left( 1 - \frac{B}{XPB_{DL} \cdot DL + XPB_{DR} \cdot DR} \right)^{k_{PB}}$

where mum is the maximum growth rate ([mum_PB]{style="color: orange;"}), XPB~DL~ and XPB~DR~ is the maximum ratio of the bacteria to DL and DR biomass ([XPB_DL]{style="color: orange;"} and [XPB_DR]{style="color: orange;"} parameters), *B* is the biomass of the pelagic bacteria, *DL* and *DR* is the biomass of labile and refractory detritus and [k_PB]{style="color: orange;"} is the constant determining the exponent of growth (1- linear, 3-drop off before maximum, 5- drop off at maximum). The growth of sediment bacteria (BB) is modelled in the same way, except that [mum_BB]{style="color: orange;"} and [k_BB]{style="color: orange;"} parameters are used.

There is an option to allow for stimulation of bacterial growth by bioirrigation and bioturbation. This option is turned on by setting [flagbactstim]{style="color: orange;"}=1 and will enhance growth depending on the biomass of infaunal organisms and porosity of the sediment (see @sec-bioirrig-bioturb).
...
where *mL* is linear mortality, *mA* is the acidification mortality, *mS* is the extra mortality due to acidification effects, *mO* is the oxygen limitation mortality, where (1-δ~O2~) is the oxygen limitation scalar, *B~PB~* is the biomass of pelagic bacteria (or BB if sediment bacteria are modelled) and *mortsc* is a forced external mortality scalar.

The uptake of detritus by pelagic bacteria is modelled as

$\frac{d(DL)}{dt} = \frac{Gr_{BP} \cdot PB_{DL}}{B_{BP} \cdot E_{DL,BP}}$

where *E~DL,PB~* is the assimilation efficiency on labile detritus ([EDL_BP]{style="color: orange;"} for pelagic bacteria and [EDL_BB]{style="color: orange;"} for benthic bacteria) and *PB_DL* is the current (box specific) partitioning of PB to DL (vs DR).

Uptake of refractory detritus DR is modelled in the same way except that PB_DR is used and assimilation efficiencies on refractory detritus are used ([EDR_BP]{style="color: orange;"} and [EDR_BB]{style="color: orange;"}).

## Biogeochemical processes 

Details on the fluxes in the inanimate pools have been described in Fulton et al. 2004a and more recently in Link et al. 2011 [Atlantis-NEUS model report](https://atlantis-ecosystem-model.github.io/atlantis-wiki/useful-reading.html). The table of inanimate pools tracked in the water column and sediments is given in @tbl-state-variables.
