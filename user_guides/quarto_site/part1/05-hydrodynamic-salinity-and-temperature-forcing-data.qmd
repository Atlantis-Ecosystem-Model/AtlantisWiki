...
| **Hydrodynamic model used to supply flows** | OFAM-Bluelink[^oke] | OFAM-Bluelink | 3D hydro model[^walker] | Empirically defined | HYCOM[^hycom] | ROMS[^romsh] | ROMS[^romsp] |
...
These three steps constitute the main conversion and are conducted using the [*Hydro*](https://atlantis-ecosystem-model.github.io/atlantis-wiki/matlab-hydro.html) Matlab script (follow the link for further instructions on installation and use of the *Hydro* script and also check [*Hydro* FAQ](https://atlantis-ecosystem-model.github.io/atlantis-wiki/hydro-faq.html) for more information). *Hydro* produces two NC files which are then used by [*Hydroconstruct*](https://atlantis-ecosystem-model.github.io/atlantis-wiki/hydroconstruct.html) to produce the final Atlantis forcing files. *Hydroconstruct* makes sure the order of water columns is in the right direction, transforms the water flux data into per layer per box fluxes and also corrects for hyperdiffusion (using either area based scaling or area and box shape based scaling). The *Hydroconstruct* [running](https://atlantis-ecosystem-model.github.io/atlantis-wiki/running-hydroconstruct.html) page and [parameter](https://atlantis-ecosystem-model.github.io/atlantis-wiki/attachments/43156182.prm) file provides further information on the script and how to run it.

![Main steps in converting oceanographic files into Atlantis input files, as done using two CSIRO contributed scripts *Hydro* and *Hydroconstruct*. Figure by Javier Porobic Garate.](media/image27.jpeg){#fig-hydro-conversion width=6.89in height=2.23in}

::: callout-note
Hydroconstruct is generally unsupported at this time. More generalized workflows are underway.
:::

::: {.callout-tip title="Good practice tip 3"} 
...
## Things to keep in mind about hydrodynamic forcing files 

Oceanographic modelling has made rapid progress in recent years and availability of high resolution data is increasing, but there are a **few issues that should be kept in mind:**

1. **Absence of long term oceanographic data to study long term trends.** Oceanographers rarely conduct 50+ years of high resolution simulations, yet such time frames are often needed for Atlantis simulations. The length of oceanographic data time series is usually more on the order of 1 to 25 years (@tbl-hydrodynamic-structure) and a common practice among Atlantis users is to 'loop' over the available oceanographic input years. One should be aware that if long term trends are present in the oceanographic data, they will be seen as jumps in average variable values every time the file is recycled. Also, such recycling alone is not suitable for studying long term environmental trends. Ideally long term forcing files should be obtained directly from long term oceanographic simulations. As a crude approximation, **long term trends can be forced** onto hydrodynamic files using an [R script](https://confluence.csiro.au/display/Atlantis/R+code%2C+add+climate+trends+to+Hydro.NC) contributed by Hem Nalini Morzaria Luna and used for the Gulf of California model. Alternatively, trends in temperature, salinity and pH can be added in the *biology.prm* file using several [Tchange]{style="color: orange;"}, [Schange]{style="color: orange;"} and [PHchange]{style="color: orange;"} parameters (see below).
...
The final flows of water are given in the output.nc file listed in [hdsource]{style="color: orange;"}, [hdsink]{style="color: orange;"}, [eflux]{style="color: orange;"} and [vflux]{style="color: orange;"} (see description of the output file parameters in @sec-output-physical-tracers). If some of the optional physics routines are used, then these final exhcanges may be different from the hydrodynamic forcing inputs.
:::

## Numbering of vertical layers in the input and output NC files 
...
|:-------------------|:-------------------|
| **Temp =** | **Temp =** |
| 15.75, 18.54, 18.55, 0, 0, 0, 11.85, | 0, 0, 0, 15.75, 18.54, 18.55, 11.85, |
| 16.38, 17.64, 17.49, 0, 0, 0, 11.77, | 0, 0, 0, 16.38, 17.64, 17.49, 11.77, |
| 15.66, 17.34, 17.31, 0, 0, 0, 11.57, | 0, 0, 0, 15.66, 17.34, 17.31, 11.57, |
| 16.05, 16.92, 16.87, 0, 0, 0, 11.29, | 0, 0, 0, 16.05, 16.92, 16.87, 11.29, |
| 5.15, 11.85, 13.92, 17.02, 19.01, 19.83, 0.32, | 5.15, 11.85, 13.92, 17.02, 19.01, 19.83, 0.32, |
| 5.29, 11.91, 13.85, 16.39, 18.50, 19.48, 0.93, | 5.29, 11.91, 13.85, 16.39, 18.50, 19.48, 0.93, |
| ... | ... |
:::

## Viewing and checking forcing input files with R {#sec-r-forcing-check}
...
- *run.prm file*: set [flag_pollutant_impacts]{style="color: orange;"} to 1
- *forcing.prm file*: set [use_pollutantfiles]{style="color: orange;"} to 1 and then add the following:

  [nnoise_pollutionfiles]{style="color: orange;"} X (where X is the number of noise forcing files)
  [Noise_Pollution0.name]{style="color: orange;"} path/noise_filename.nc
  [noise_pollution_rewind]{style="color: orange;"} 1 (set to 0 if you do not want the file(s) to be rewound and reused)

  [nlight_pollutionfiles]{style="color: orange;"} X (where X is the number of light pollution forcing files)
  [Light_Pollution0.name]{style="color: orange;"} path/light_pollution_filename.nc
  [light_pollution_rewind]{style="color: orange;"} 1 (set to 0 if you do not want the file(s) to be rewound and reused)

- *biology.prm* *file*: Add the parameters [noise_coefft_XX]{style="color: orange;"} and [light_coefft_XX]{style="color: orange;"} for each biological group
- *groups.csv:* Add the columns [isLightEffected]{style="color: orange;"} and [isNoiseEffected]{style="color: orange;"} and set the value to 1 for these if you want noise/light pollution to affect that species group
...
Where $\mathbb{N}_{i,j}$ represents the level of the pollutant (noise or light) in layer $j$ of polygon (box) $i$; and $\kappa_{\eta,s}$ is the coefficient for that pollutant type.

In addition, metabolic parameters — consumption ([C]{style="color: orange;"}) and growth rates ([mum]{style="color: orange;"}) — are scaled in the same way; while starvation and background ([mL]{style="color: orange;"}, [mQ]{style="color: orange;"}) mortality rates are scaled by the inverse.