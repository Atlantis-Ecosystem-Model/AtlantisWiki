...
TAC-based management can be applied to both F-mortality based fisheries and effort-based fisheries, with the exception of forced effort fisheries. For F-mortality based fisheries, TAC-based management is enabled using the flag [flag_stop_F_tac]{style="color: orange;"}. The parameterization described in the sections above applies equally to R-based TAC management.
...
dfTAC <- data.frame(fishery_id = 0:16, TAC = c(totcatch*0.8*1000,rep(0,16)))
...
| [RAssessRuseScript]{style="color: orange;"} | Controls how Atlantis calls R. Set to 1 to run external R scripts via system calls, or 0 to use embedded R calls directly within Atlantis. |
